const locationOrigin = window.location.hostname + (window.location.port ? ':' + window.location.port: '');
...
function validateLoginForm(e) {
        ...
        ipAddress = $('#ip').val() + "/api/v2/bot";
        location1 = ipAddress;

        const checkConnectionType = $('#selectConnectionType').val();
        console.log(checkConnectionType)
        if(checkConnectionType === 'websocket') {
            isWebSocketBase = true;
            isHttpBase = false;
        } else {
            isWebSocketBase = false;
            isHttpBase = true;
        }

        if(users_list.indexOf(username)) {
            credentialPass();
        }
        else {
    ...
    /*------------ Function Validation pass start ----------------- */
        function credentialPass() {
            ...
            if ($("#selectLanguage_new").val() == 'hi') {
                $(".welcome-content h3").html("नमस्ते, " + msg);
                $(".welcome-content h2").html("मैं एरिया हूं, मैं आपकी मदद कैसे कर सकती हूं?");
            }
            else if($("#selectLanguage_new").val() == 'mr') {
                $(".welcome-content h3").html("नमस्ते, " + msg);
                $(".welcome-content h2").html("मी आरिया आहे, मी आपली कशी मदत करू शकतो?");
            }
            else {
                $(".welcome-content h3").html("Hello, " + msg);
                $(".welcome-content h2").html("I am ARIA, How may I help you?");
            }
        ...
        function initialCall() {
            if(isWebSocketBase) {
                /* --------------- New web socket connection code Start ---------------------*/
                let wsProto;
                ...
                ws = new WebSocket(wsProto+locationOrigin+"/api/v2/msg");
                //ws = new WebSocket("wss://"+locationOrigin+"/api/v2/msg");
                ...
                ws.onmessage = function (e) {
                    ...
                    if((data.result.reply).includes('systemNotitification:')) {
                        ...
                        systemUtteranceContent = '<li class="mar-btm systemUtterance notification"> <div class="media-body pad-hor"> <div class="speech"> <p id="systemUtterance">'+ dataMsg + '</p><p class="speech-time"><i class="fa fa-clock-o fa-fw"></i>' + currentTime() + '</p> </div></div></li>';
                    } else {
                        let dataMsg;
                        if((data.result.query).startsWith('@')) {
                            dataMsg  = data.result.reply;
                        } else {
                            dataMsg = msg + ', ' + data.result.reply;
                        }
                        systemUtteranceContent = '<li class="mar-btm systemUtterance"> <div class="media-body pad-hor"> <div class="speech"> <p id="systemUtterance">' + dataMsg + '</p><p class="speech-time"><i class="fa fa-clock-o fa-fw"></i>' + currentTime() + '</p> </div></div></li>';
                    }
                        $('#loading').hide();
                ...
                /* --------------- New web socket connection code End ---------------------*/
            ...
            $.post(location1, {
                    ...
                    console.log(locationInt + ' Location Header');
                    getCurrentTask();
